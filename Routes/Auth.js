...
// Order data route to store order data including images
router.post('/order_data', async (req, res) => {
    let data = req.body.order_data;
    await data.splice(0, 0, { Order_date: req.body.order_date });

    // Ensure each order item includes its image URL
    data = data.map(item => ({
        ...item,
        img: item.img || 'defaultImageURL'  // Ensure there is a fallback image if not provided
    }));
